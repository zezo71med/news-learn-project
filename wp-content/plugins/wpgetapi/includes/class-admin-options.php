...
class WpGetApi_Admin_Options {
		...
		$endpoint_fields = apply_filters( 'wpgetapi_fields_endpoints', array(
			...
			array(
			    'name' => __( 'Timeout', 'wpgetapi' ) . 
				    '<span class="dashicons dashicons-editor-help" data-tip="How long the connection should stay open in seconds. Default is 10. If you receive curl 28 errors, try increasing this value."></span>',
	            'id'   => 'timeout',
	            'type' => 'text',
	            'default' => '10',
			...
			array(
                'name' => __( 'Actions', 'wpgetapi' ) . '<span class="dashicons dashicons-editor-help" data-tip="' . 
			    		sprintf( 
	            			__( 'Call this endpoint when your chosen action runs. This feature is available in the PRO plugin. %1s', 'wpgetapi' ),
	...
	public function admin_page_display() {
		...
		<div class="wrap wpgetapi <?php esc_attr_e( $l ); ?>">
	...
	public function get_import_export() {
		...
		<div class="wpgetapi-export">
			<h3><span class="dashicons dashicons-download"></span> Export Endpoints</h3>
			<p>Export the endpoints for this API.<br>After clicking the button below, copy the entire contents of the export.</p>
			<button class="button button-primary">Export Endpoints</button>
			<div class="message"></div>
		</div>

		<div class="wpgetapi-import">
			<h3><span class="dashicons dashicons-upload"></span> Import Endpoints</h3>
			<p>Paste your exported JSON data into the field below to import endpoints into this API.</p>
			<textarea></textarea>
			<button class="button button-primary">Import Endpoints</button>
			<div class="message"></div>
		</div>
	...
	public function import_endpoints() {
		...
		if( ! current_user_can( 'manage_options' ) ) {
			echo 'Admin privelages are required to import endpoints.';
			wp_die();
		}

		if ( isset( $_POST['nonce'] ) ) {
			if ( ! wp_verify_nonce( $_POST['nonce'], 'import_endpoint_nonce' ) ) {
				echo 'Nope.';
				wp_die();
			}	
		}
