...
class CMB2 extends CMB2_Base {
	...
	public function render_group_row( $field_group ) {
		...
		$group_title = isset( $field_group->value[$field_group->index]['id'] ) ? $field_group->value[$field_group->index]['id'] : $field_group->replace_hash( $field_group->options( 'group_title' ) );
		...
		if ( $field_group->args( 'repeatable' ) ) {
			...
			$button = isset( $_GET['page'] ) && $_GET['page'] == 'wpgetapi_setup' ? '' : '<button class="test-endpoint-button button-primary">' . esc_html__( 'Test Endpoint', 'wpgetapi' ) . '</button>';
			
			echo '
			<div class="cmbhandle" title="' , esc_attr__( 'Click to toggle', 'cmb2' ), '"><br></div>
			<h3 class="cmb-group-title cmbhandle-title"><span>', $group_title, '</span>' . $button . '</h3>
